...
func TestMapMatch_FormBody(t *testing.T) {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		json.NewEncoder(w).Encode(mapbox.MapMatchResponse{Code: mapbox.CodeOK})
	}))
	defer srv.Close()
	...
	if tok := gotReq.URL.Query().Get("access_token"); tok != "test-token" {
		t.Errorf("expected access_token in URL query, got %q", tok)
	}
...
func TestMapMatch_NoMatch_NotAnError(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "application/json")
		json.NewEncoder(w).Encode(mapbox.MapMatchResponse{
	...
	if err != nil {
		t.Fatalf("expected nil error for NoMatch, got %v", err)
	}
	if resp.Code != mapbox.CodeNoMatch {
		t.Errorf("expected Code %q, got %q", mapbox.CodeNoMatch, resp.Code)
	}
	if resp.Code.IsSuccess() {
		t.Error("NoMatch.IsSuccess() should be false")
	}
}
...
func TestMapMatch_HTTPError(t *testing.T) {
	...
	if !mapbox.IsInvalidInput(err) {
		t.Errorf("expected IsInvalidInput(err) = true, got false; err = %v", err)
	}
}
...
func TestEncodeCoordinates(t *testing.T) {
	got := encodeCoordinatesForTest([]mapbox.Coordinate{
		{Longitude: 13.4, Latitude: 52.5},
...
func encodeCoordinatesForTest(coords []mapbox.Coordinate) string {
	var captured string
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		body, _ := io.ReadAll(r.Body)
	...
	defer srv.Close()
	client := mapbox.NewClient(mapbox.WithAccessToken("t"), mapbox.WithBaseURL(srv.URL))
	_, _ = client.MapMatch(context.Background(), &mapbox.MapMatchRequest{Coordinates: coords})
	return captured
}
