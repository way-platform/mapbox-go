...
type MapMatchRequest struct {
	...
	// ADR §1 recommends 25m as a starting value.
	Radiuses []float64
...
type Matching struct {
	...
	// Mapbox treats values below 0.5 as low-quality (ADR §1).
...
// response.Code and implement the degradation policy described in ADR §1
// (fall back to the raw input path when Code is not Ok or Confidence < 0.5).
//
